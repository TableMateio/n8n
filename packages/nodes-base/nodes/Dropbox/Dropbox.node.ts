	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				description:
					'Whether to create a temporary shareable link for the uploaded file (expires in 4 hours)',
			},
			{
				...
				if (resource === 'file' && operation === 'upload') {
					...
					if (createShareableLink) {
						try {
							...
							// Use Dropbox's get_temporary_link API (works with Plus accounts, expires in 4 hours)
							const filePath = data.path_display || data.path_lower;

							console.log('🔗 [DEBUG] Getting temporary link for file:', filePath);

							// Call the get_temporary_link API
							const tempLinkResponse = await dropboxApiRequest.call(
								this,
								'POST',
								'https://api.dropboxapi.com/2/files/get_temporary_link',
								{ path: filePath },
								{},
								{}, // Clean headers for JSON API
							);

							console.log('🔗 [DEBUG] ✅ Temporary link response:', tempLinkResponse);

							// The response contains { metadata: {...}, link: "https://..." }
							// The link already comes with dl=1 (direct download) by default
							...
							// Note: Temporary links come with dl=1 by default (direct download)
							let shareableUrl = sharedLinkResponse.url;
							if (linkType === 'preview') {
								// Change dl=1 to dl=0 for preview page behavior
								shareableUrl = shareableUrl.replace('dl=1', 'dl=0');
							}
							// For 'direct' type, keep dl=1 (default from temporary link)
							...
							data.link_expires_in = '4 hours';
							data.link_method = 'temporary_link';
							...
							console.log('🔗 [DEBUG] FULL ERROR OBJECT:', JSON.stringify(linkError, null, 2));

							// Show the REAL Dropbox error, not my interpretation
							data.shareable_url_error = `REAL DROPBOX ERROR: ${linkError.message}`;
						}
					}
